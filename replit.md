...
The application features a comprehensive multi-language translation system powered by react-i18next with HTTP backend, providing full localization for English, Hausa, Yoruba, and Igbo. Locale files are stored in `client/public/locales/` (which Vite automatically copies to the build output) and loaded dynamically via HTTP requests. The LanguageSwitcher component (native `<select>` dropdown) is positioned in the header between UserProfile and ThemeToggle on authenticated pages, and in the top-right corner (fixed position) on login/register pages, with responsive styling that adapts to light/dark themes. Language preferences persist to localStorage and update instantly across the entire application without page reload.

**Complete i18n Implementation:**
- **HTTP Backend Loading**: Locale files stored in `client/public/locales/` (en.json, ha.json, yo.json, ig.json) are automatically copied to the production build by Vite and loaded dynamically via `i18next-http-backend`
...
- **Auth Pages Localization**: Login and register pages fully translated with language switcher visible (fixed top-right position)
...
- **Translation Structure**: Organized JSON files at `client/public/locales/{lang}.json` with nested keys for logical grouping (nav, common, auth, dashboard, sales, expenses, etc.)
- **Pluralization Support**: Uses i18next pluralization for dynamic counts (e.g., "1 product" vs "2 products" in all languages)
- **Instant Updates**: Language switching triggers immediate re-render of all components without page reload, updating sidebar navigation, form labels, dashboard content, auth pages, and all other UI elements simultaneously
- **Build-Ready**: Locale files in `public/` folder ensure translations are included in production builds automatically
